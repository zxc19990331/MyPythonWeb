...
#带导航栏的博客页面（PS教程）
@get('/ps/{id}')
def get_sidebar_blogs(id):
    blog = yield from Blog.find(id)
    num = yield from Blog.findNumber("count(id)","tag='ps'")
    if num == 0:
        blogs = []
    else:
        blogs = yield from Blog.findAll(where = "tag='ps'",orderBy = "created_at")
    comments = yield from Comment.findAll('blog_id=?', [id], orderBy='created_at desc')
    # 将每条评论都转化为html格式(根据text2html代码可知,实际为html的<p>)
    for c in comments:
        c.html_content = text2html(c.content)
    ...
    # app.py的response_factory将会对handler的返回值进行分类处理
    return {
        "__template__": "blogs_PS.html",
        "blogs_PS": blogs, # 参数blogs将在jinja2模板中被解析
        "blog": blog,
        "comments": comments
    }
...
def api_register_user(*,name, email, passwd): # 注册信息包括用户名,邮箱与密码
    ...
    r.set_cookie(COOKIE_NAME, user2cookie(user, 6000), max_age=6000, httponly=True)  # 设置cookie最大存会时间为10min
...
def authenticate(*, email, passwd): # 通过邮箱与密码验证登录
    ...
    r = web.Response()
    r.set_cookie(COOKIE_NAME, user2cookie(user, 6000), max_age=6000, httponly=True)
