...
# 创建应用时,通过指定一些命名关键字为"middle factory"的列表可创建中间件Middleware
...
def init(loop):
    # 创建全局数据库连接池
    yield from orm.create_pool(loop = loop, host="127.0.0.1", port = 3306, user = "www-data", password = "123456", db = "awesome", autocommit = True)
    # 创建web应用,
